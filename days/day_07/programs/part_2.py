...
class Part_2(BaseClass):
    ...
    def convert_card(self, cards):
        ...
        card_set_frequency = [[x, cards.count(x)] for x in card_set]

        card_set_frequency.sort(key=lambda x: x[1], reverse=True)

        if self.joker in card_set and len(card_set) > 1:
            # There's a joker and at least another card we can replace it with

            joker_frequency_item = [x for x in card_set_frequency if x[0] == self.joker][0]
            joker_index = card_set_frequency.index(joker_frequency_item)

            # Remove the joker from the list of cards, and count it as the next most common card
            # (add its counter to the first card in the ordered list)
            del card_set_frequency[joker_index]
            card_set_frequency[0][1] += joker_frequency_item[1]

        card_set_frequency = [x[1] for x in card_set_frequency]
