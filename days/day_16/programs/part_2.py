...
class Part_2(BaseClass):
    ...
    def execute_internal(self, filepath):
        ...
        starting_beam_set = set()
        for cur_col in range(cols):
            # Beams heading south
            starting_beam_set.add((0, cur_col, Direction.SOUTH))
            # Beams heading north
            starting_beam_set.add((rows - 1, cur_col, Direction.NORTH))
        for cur_row in range(rows):
            # Beams heading east
            starting_beam_set.add((cur_row, 0, Direction.EAST))
            # Beams heading west
            starting_beam_set.add((cur_row, cols - 1, Direction.WEST))

        max_energy = 0

        for starting_beam in starting_beam_set:
            energized_matrix = np.zeros(matrix.shape)
            beam_set = {starting_beam}
            ...
            cur_energy = int(energized_matrix.sum())
